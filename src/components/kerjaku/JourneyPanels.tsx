      ...
      {panel && (
        ...
        >
          ...
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-label={panel === "work" ? "Panel karya" : "Panel proses"}
            initial={{ y: 40, opacity: 0, filter: "blur(10px)" }}
            animate={{ y: 0, opacity: 1, filter: "blur(0px)" }}
            exit={{ y: 30, opacity: 0, filter: "blur(10px)" }}
            transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
            className="relative m-0 max-h-[88svh] w-full max-w-3xl overflow-y-auto rounded-t-[2rem] glass-panel p-6 sm:m-6 sm:rounded-[2rem] sm:p-8"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <p className="text-[11px] uppercase tracking-[0.32em] text-primary/90">
                  {panel === "work" ? `Work · ${mode}` : "Build process"}
                </p>
                <h2 className="mt-3 font-display text-3xl leading-tight">
                  {panel === "work" ? "Peta Karya Digital" : "Cara Saya Membangun"}
                </h2>
              </div>
              <OceanButton
                variant="secondary"
